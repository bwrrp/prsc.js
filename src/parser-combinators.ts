...
function getUtf16Length(cp: number): number {
...
export function codepoint(
	...
	return (input: string, offset: number) => {
		...
		return ok(offset + getUtf16Length(cp));
	};
}
 ...
 */
export function map<T, U, Input = string>(
 ...
 */
export function filter<T, Input = string>(
	parser: Parser<T, Input>,
	filter: (v: T) => boolean,
	expected: string[],
	fatal?: boolean
 ...
 */
export function or<T, Input = string>(
	parsers: Parser<T, Input>[],
	expected?: string[]
): Parser<T, Input> {
	return (input, offset) => {
 ...
 */
export function filterUndefined<T, Input = string>(
 ...
 */
export function then<T1, T2, T, Input = string>(
	parser1: Parser<T1, Input>,
	parser2: Parser<T2, Input>,
 ...
 */
export function sequence<Ts extends unknown[], Input = string>(
	...parsers: { [key in keyof Ts]: Parser<Ts[key], Input> }
): Parser<Ts, Input> {
	return (input, offset) => {
 ...
 * @param parsers - Parsers to apply one after the other
 */
export function sequenceConsumed<Input = string>(
	...parsers: Parser<unknown, Input>[]
): Parser<void, Input> {
	return (input, offset) => {
		for (const parser of parsers) {
			const res = parser(input, offset);
 ...
 */
export function preceded<TBefore, T, Input = string>(
	before: Parser<TBefore, Input>,
	parser: Parser<T, Input>
): Parser<T, Input> {
 ...
 */
export function followed<T, TAfter, Input = string>(
	parser: Parser<T, Input>,
	after: Parser<TAfter, Input>
): Parser<T, Input> {
 ...
 */
export function delimited<TOpen, T, TClose, Input = string>(
	open: Parser<TOpen, Input>,
	inner: Parser<T, Input>,
	close: Parser<TClose, Input>,
 ...
 */
export function not<T, Input = string>(
	parser: Parser<T, Input>,
	expected: string[]
): Parser<void, Input> {
	return (input, offset) => {
 ...
 */
export function except<T, U, Input = string>(
	match: Parser<T, Input>,
	except: Parser<U, Input>,
	expected: string[]
): Parser<T, Input> {
...
export function dispatch<T>(
	mapping: { [codepoint: number]: Parser<T, string> },
	otherwise: Parser<T, string> | undefined,
 ...
 */
export const start: Parser<void, unknown> = (_input, offset) =>
 ...
 * @param offset - The offset in `input` at which to start matching
 */
export const end: Parser<void, { length: number }> = (input, offset) =>
 ...
 * @param parser - The parser to wrap
 */
export function complete<T, Input extends { length: number } = string>(
	parser: Parser<T, Input>
): Parser<T, Input> {
